...
function findSingleLoggedInAgent(): string | null {
  try {
...
function sharedBaseHasAuth(): boolean {
  try { return existsSync(join(STATE_BASE, 'auth.json')) } catch { return false }
}
...
function resolveAgentKey(): string {
  ...
  if (!sharedBaseHasAuth()) return findSingleLoggedInAgent() ?? ''
  return ''
}
...
export type BindingSource = 'env' | 'local-file' | 'auto-discovered' | 'default-shared'
...
export async function ensureAgentBinding(_create: boolean): Promise<AgentBinding> {
  const env = (process.env.SIOBAC_AGENT_KEY ?? process.env.OVOCLAW_AGENT_KEY ?? '').trim()
  if (env) {
...
function discoverSkipsFile(): string { return join(dir(), 'discover-skips.json') }
export async function bumpDiscoverSkips(): Promise<number> {
  let count = 0
  try {
...
export async function resetDiscoverSkips(): Promise<void> {
  try { await fs.unlink(discoverSkipsFile()) } catch (e) {
    if ((e as NodeJS.ErrnoException).code !== 'ENOENT') { /* ignore — non-critical */ }
  }
}
