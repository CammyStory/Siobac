...
async function cmdLogin(flags) {
    ...
    if (isModelSuggestionPlatform() && !modelChecked(flags)) {
        ok({
            ...
            message: 'FIRST STEP — before any login link: check which model YOU are running. ' +
            ...
            next_step: 'Do NOT show a login link in this step — there is none yet. After the user has responded to the model suggestion ' +
...
async function cmdLoginFinish(_flags) {
    ...
    let portableToken = null;
    let portableLogin = null;
...
async function cmdIssuePortableLogin(_flags) {
    ...
    ok({
        ...
        next_step: `A PORTABLE login is minted for ${r.agent_name ?? 'this agent'} — a non-rotating token valid ${days} days. ` +
...
async function cmdRevokePortable(_flags) {
    const { auth } = await requireBoundAgent();
...
async function cmdRecall(flags) {
    ...
    let prof = null;
    try {
        prof = await api.getAgentProfile(auth.accessToken, agentId);
    }
    catch { /* best-effort */ }
    ok({
        status: 'ok',
...
async function cmdCheck(_flags) {
    ...
    if (auth && auth.agentId) {
        ...
        try {
            ...
            if (dsc.suggestion)
                result.discovery = { suggestion: dsc.suggestion };
        }
...
// the refine leads.
function skipCheckinNextStep(s, skips) {
    return (`The owner has hit "next" ${skips} times IN A ROW. Read this as a signal the recommendations are NOT landing — ` +
...
async function cmdDiscover(flags) {
    const boolFlag = (v) => v === true || v === 'true' || v === '';
    const { auth, agentId } = await requireBoundAgent();
    ...
    if (boolFlag(flags.connect)) {
        ...
        const r = await api.acceptSuggestion(bearer, agentId, manual ? { icebreak: 'manual', introduction: hello } : {});
        if (!r.ok) {
            return ok({
        ...
        const handle = r.connection_id
            ?? r.session?.connection_id
            ?? undefined;
        return ok({
