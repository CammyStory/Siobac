...
import { loadAuth, saveAuth } from './state.js';
// ── Output contract ────────────────────────────────────────────────────
...
export function updateInstruction(repoUrl) {
...
export function enrichNotice(upd) {
...
export function withUpdateNotice(body) {
...
export function ok(value, withUpdate = false) {
    const payload = withUpdate && value && typeof value === 'object' && !Array.isArray(value)
...
export function isConfirmed(flags) {
...
function reauthMessage(fallback) {
...
export async function requireAuth() {
    ...
    if (!auth) {
        throw api.makeApiError('not_authenticated', reauthMessage('no auth.json found. Run `login` first to authenticate via device flow. (Ephemeral-workspace host? Use `issue-portable-login` and set SIOBAC_TOKEN — see `references/platform-hints.md`.)'));
    }
    ...
    if (!auth.refreshToken) {
        throw api.makeApiError('session_expired', reauthMessage('access token expired and no refresh token is stored. Run `login` to re-authenticate.'));
    }
    let token;
    try {
    ...
    catch { /* ephemeral/read-only FS — token still valid in-memory */ }
    return refreshed;
}
...
export async function requireBoundAgent() {
...
export function qrMarkdownFor(slug) {
...
export async function verifyShareResolves(slug, expectedName) {
    try {
        ...
        const code = e.code;
        return { resolves: false, points_back: false, reason: code ?? e.message };
    }
}
