...
export async function cmdGuide(flags) {
    ...
    ok({
        status: 'ok',
        overview: 'Operating procedure (which command, when). For the LIVE next action use each command\'s `next_step`. OWNER-FACING WORDING is NOT here — compose it from references/scripts-en.md / scripts-cn.md per references/brain.md → Inward (short, human, numbered options).',
        steps: GUIDE_STEPS,
    });
}
// ── Help (JSON) ──────────────────────────────────────────────────────
export function cmdHelp() {
    ok({
        ...
        description: 'siobac — one agent, both directions on Siobac (咻叭): be reached by others AND reach out to others. Run `guide` for the operating procedure; every command returns `next_step` to drive the flow. Owner-facing wording comes from references/scripts-en.md / scripts-cn.md (per references/brain.md → Inward), NOT from the JSON.',
